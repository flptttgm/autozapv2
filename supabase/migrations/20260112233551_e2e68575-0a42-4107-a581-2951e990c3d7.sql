-- 1. Atualizar o trigger handle_new_user para copiar whatsapp_number
...
-- 2. Corrigir usuários existentes que têm WhatsApp no metadata mas não no profile
UPDATE profiles p
SET whatsapp_number = u.raw_user_meta_data->>'whatsapp_number'
FROM auth.users u
WHERE p.id = u.id
  AND (p.whatsapp_number IS NULL OR p.whatsapp_number = '')
  AND u.raw_user_meta_data->>'whatsapp_number' IS NOT NULL
  AND u.raw_user_meta_data->>'whatsapp_number' != '';
-- Update handle_new_user function to create agent_routing_config with routing enabled by default
...
BEGIN
  ...
  -- Create profile (CORRIGIDO: inclui whatsapp_number)
  ...
  -- Create default agents (usando jsonb para trigger_keywords)
  -- Agent 1: Vendedor
  ...
  -- Agent 2: Atendente (support - será o agente padrão)
  INSERT INTO public.custom_templates (
    workspace_id, name, agent_type, agent_persona_name, 
    description, icon, trigger_keywords, transition_message, config
  ) VALUES (
  ...
  INSERT INTO public.agent_routing_config (
    ...
    new_workspace_id,
    true,              -- Roteamento ATIVO por padrão
    'hybrid',          -- Modo híbrido
    'friendly',        -- Estilo amigável
    0.70,              -- Threshold de 70%
    support_agent_id   -- Luana (Atendente) como agente padrão
  );
